...
export const DrawingCanvas: React.FC<DrawingCanvasProps> = ({
  ...
  return (
    <div className="flex flex-col w-full h-full select-none">
      {/* Canvas Area */}
      <div className="relative flex-1 bg-chamber-bg border border-chamber-cyan/20 rounded-lg overflow-hidden w-full h-full flex items-center justify-center min-h-0">
        <canvas
          ref={canvasRef}
          width={800}
          height={600}
      ...
      {isDrawer && (
        <div className="mt-1.5 md:mt-2 p-1 md:p-2 hologram-panel rounded-lg flex flex-row items-center justify-between gap-1 md:gap-3 z-10 shrink-0 w-full min-w-0 overflow-hidden">
          {/* Colors */}
          <div className="flex items-center gap-1 shrink-0">
            {COLORS.map(c => (
              <button
                ...
                className={`w-5 h-5 md:w-7 md:h-7 rounded-full border transition-transform shrink-0 ${
                  color === c && !isEraser
            ...
            <button
              ...
              className={`w-5 h-5 md:w-7 md:h-7 rounded-full flex items-center justify-center border transition-all shrink-0 ${
                isEraser
              ...
              title="Eraser"
            >
              <Eraser size={10} className="md:w-3.5 md:h-3.5" />
            </button>
          </div>
          ...
          <div className="flex items-center gap-1 shrink-0">
            {BRUSH_SIZES.map(size => (
              <button
                ...
                className={`w-5 h-5 md:px-2 md:py-0.5 rounded text-[8px] md:text-xs font-cyber tracking-wider border transition-colors flex items-center justify-center shrink-0 ${
                  brushSize === size
          ...
          <div className="flex items-center gap-1 shrink-0">
            <button
              ...
              className="w-5 h-5 md:p-1.5 rounded border border-chamber-cyan/10 hover:border-chamber-cyan/30 text-chamber-secondary hover:text-chamber-cyan disabled:opacity-30 disabled:pointer-events-none transition-colors flex items-center justify-center shrink-0"
              title="Undo Last Stroke"
            >
              <Undo size={11} className="md:w-4 md:h-4" />
            </button>
            <button
              onClick={handleClear}
              disabled={localHistory.length === 0}
              className="w-5 h-5 md:p-1.5 rounded border border-chamber-red/10 hover:border-chamber-red/30 text-chamber-secondary hover:text-chamber-red disabled:opacity-30 disabled:pointer-events-none transition-colors flex items-center justify-center shrink-0"
              title="Clear Canvas"
            >
              <Trash2 size={11} className="md:w-4 md:h-4" />
            </button>
          </div>
        </div>
      )}
