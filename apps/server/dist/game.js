...
exports.clearSessionTimers = clearSessionTimers;
exports.generateChamberId = generateChamberId;
exports.serializeSession = serializeSession;
exports.saveSessionToRedis = saveSessionToRedis;
exports.createChamber = createChamber;
exports.generateHints = generateHints;
exports.startWordSelection = startWordSelection;
exports.selectWord = selectWord;
exports.endRound = endRound;
exports.handleGuessScore = handleGuessScore;
...
const cleanupInterval = setInterval(() => {
    const now = Date.now();
    for (const [chamberId, session] of exports.activeSessions.entries()) {
        if (now - session.lastActiveTime > 30 * 60 * 1000) {
            console.log(`Chamber ${chamberId} expired due to inactivity.`);
            clearSessionTimers(session);
            exports.activeSessions.delete(chamberId);
...
function clearSessionTimers(session) {
    if (session.wordSelectTimeout)
        clearTimeout(session.wordSelectTimeout);
    if (session.drawingTimeout)
        clearTimeout(session.drawingTimeout);
    if (session.resultsTimeout)
        clearTimeout(session.resultsTimeout);
    session.wordSelectTimeout = null;
    session.drawingTimeout = null;
    session.resultsTimeout = null;
}
function generateChamberId() {
    ...
    // Check collision
    if (exports.activeSessions.has(code)) {
        return generateChamberId();
    }
    return code;
}
function serializeSession(session, includeCanvas = false) {
    const state = {
...
async function saveSessionToRedis(session) {
    ...
    await (0, redis_1.setVal)(`chamber:${session.chamberId}`, JSON.stringify({
        ...state,
...
function createChamber(chamberId, hostSocketId, codename) {
    const host = {
        id: hostSocketId,
        codename,
        score: 0,
    ...
    exports.activeSessions.set(chamberId, session);
    saveSessionToRedis(session);
    return session;
}
function generateHints(word, revealedIndices) {
    let hints = '';
...
function startWordSelection(session, broadcastState) {
    clearSessionTimers(session);
...
function selectWord(session, playerId, word, broadcastState) {
    if (session.phase !== 'WORD_SELECTION' || session.drawerId !== playerId)
        return;
    clearSessionTimers(session);
...
function revealHintLetter(session) {
    if (!session.chosenWord)
        return;
    const letters = [];
...
function endRound(session, broadcastState) {
    clearSessionTimers(session);
